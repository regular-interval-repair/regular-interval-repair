...
def exact_repair(t, lmd_a=10, lmd_d=10, interval_granularity=1, start_point_granularity=1):
    ...
    while True:
        d = 0
        while (d == 0 or check_st_lb(d, eps_list, min_cost, lmd_d, eps_t)) and d < len(eps_list):
            s_0 = t[d]
            flag_increase = False
            flag_decrease = False
        ...
        if not min_cost_change or (not check_interval_lb(eps_t, min_cost, eps_list)):
            break
        if (eps_t + interval_granularity) not in eps_t_traversed:
